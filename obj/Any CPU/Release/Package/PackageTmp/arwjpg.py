...
def convert_raw(source, target, rawpy_params):
    ...
    :param rawpy_params: parameters for rawpy
...
def get_target_files(target_dir, files, ext='JPG'):
    ...
    """
    target_files = [join(target_dir, f).replace('.ARW', '.'+ext) for f in files]
...
def main(img_dir):
    with set_directory(img_dir):
        ext = 'jpg'
        args = argparse.Namespace(bright=1.0,
                                  dcb_enhance=False,
                                  ...
                                  output_color='sRGB',
                                  source=img_dir,
                                  target=os.path.join(img_dir, ext),
                                  use_auto_wb=False,
                                  use_camera_wb=False,
                                  verbosity=0)
        ...
        results = Parallel(n_jobs=n_jobs, verbose=verbosity)(
            delayed(convert_raw)(tup[0], tup[1], rawpy_params) for tup in tqdm(tups))
        n_successes = sum(results)
        n_conversions = len(tups)
        print(f'{n_successes} of {n_conversions} successful')
